...
def create_folder(path, package_name):
    ...
    try:
        ...
        logging.printError("Could not create folder \"{}\" due to following error: {}".format(path, error), package_name)
    else:
        logging.printVerbose("Folder doesn't exist and was created: {}".format(path), package_name)
...
def create_base_folder(path, package_name):
    ...
    if basePath.exists():
        ...
        if basePath.is_file() or basePath.is_symlink():
            logging.printError("Base directory is a file or link: {}".format(basePath), package_name)
            return False
    ...
    try:
        ...
        logging.printError("Could not create parent folder \"{}\" due to following error: {}".format(basePath, error), package_name)
        return False
    else:
        logging.printVerbose("Parent folder dosent exist and was created: {}".format(basePath), package_name)
